    ...
    $.fn.chosenIcon = function (options) {
        ...
        return this.each(function () {
            ...
            // 4. Change image on chosen selected element when form changes
             $select.on('chosen:hiding_dropdown', onselectChanged);



            
            function onselectChanged (evt, params) {
                var iconClassName = ($select.find('option:selected').attr('data-icon'))
                    ? $select.find('option:selected').attr('data-icon') : null;

                if (iconClassName) {
                    var iconContent = $.getCSSValue('.' + iconClassName, 'content', ':before');
                     params.chosen.container.find('.chosen-single span').attr('data-icon', iconContent);
                }

            }
            onselectChanged (null,{chosen:{container : $chosen}}); // C'st un peu pourri mais vite fait...
        });
    }
